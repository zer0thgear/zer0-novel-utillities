...
- **Img2Img, Inpaint and Edit sources have the stealth bits erased first.** When NovelAI's client loads an image into its canvas (which is how an Img2Img base, Inpaint and Edit get their image), it checks for the stealth marker. If it finds one, it maps alpha 254 to 255 and 1 to 0, then re-encodes the image as PNG. Otherwise it leaves the image alone, as it does JPEGs and images over 16,777,216 px. Enhance and Variations send the history image untouched (`getBase64Image` straight from storage). This was confirmed on 2026-09-18 from the bundle, and from the Img2Img request it actually sent for a pasted test image: 882 pixels at alpha 254 and 10 at alpha 1 came out as 255 and 0. That request also shows the image travelling as its own multipart `image` part (the JSON holds `"image": "image"`) and, after the first upload, as an `image_cache_secret_key` instead. This app sends base64 in the JSON, which the API still accepts.
